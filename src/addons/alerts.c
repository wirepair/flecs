    ...
    const ecs_alert_desc_t *desc)
{
    ...
    ecs_filter_t *q = ecs_rule_init(world, &private_desc);
    if (!q) {
    ...
    for (i = 0; i < 4; i ++) {
        if (desc->severity_filters[i].with) {
            ...
            *sf = desc->severity_filters[i];
            if (sf->var) {
                sf->_var_index = ecs_rule_find_var(q, sf->var);
                if (sf->_var_index == -1) {
    ...
    if (desc->member) {
        ...
        int32_t var_id = 0;
        if (desc->var) {
            var_id = ecs_rule_find_var(q, desc->var);
            if (var_id == -1) {
