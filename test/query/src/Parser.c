...
static
int term_count(ecs_query_t *f) {
    test_assert(f != NULL);
    return f->term_count;
}

static
ecs_term_t* query_terms(ecs_query_t *f) {
    test_assert(f != NULL);
    return f->terms;
}
...
void Parser_component_implicit_subject(void) {
    ...
    test_int(term_count(f), 1);

    ecs_term_t *terms = query_terms(f);
